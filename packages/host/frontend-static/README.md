...
Caching follows the dist's naming: a content-hash-shaped asset name (a Vite-style `-hash` segment before the extension) serves `Cache-Control: public, max-age=31536000, immutable`, while other files carry no cache directive. The index serves `Cache-Control: no-cache` with a weak ETag over its rendered body and answers a matching If-None-Match with 304. Files at or above 1 MiB stream from disk with an explicit content length instead of buffering whole.
