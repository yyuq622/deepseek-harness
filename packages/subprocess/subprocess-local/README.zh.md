...
一次 spawn 会构建清理后的子进程环境、启动 detached 进程、把收集器挂到收集模式的流上，然后返回句柄。`done` 在进程关闭后、经过一段有界管道排空宽限期才结算，因此继承了管道的存活后代无法无限期拖住结果；Windows 终止是一次身份围栏清扫——存活根的整棵树，或已自行退出的根仍可通过父链接到达的幸存后代——经异步 taskkill spawn 投递，使取消档位绝不阻塞事件循环，同步形式仅保留给宿主退出阶段；升级定时器在直接子进程结算后依然存活，直到一次扫描确认再无可达目标。终端清理按精确身份清扫后代、停止 shell、再次清扫，并通过进程表验证其已不存在。

### 安全不变式

spill 文件以 `0600` 权限、`O_EXCL` 与随机名称在 `0700` 每进程目录下创建，可抵御共享临时目录中的符号链接植入；最终关闭失败时不公布 spill 路径。进程身份携带启动时间，因此清理绝不会跟随 PID 复用；Windows 进程树终止会在每个候选的 taskkill 之前再次核验其启动身份。运行时会登记自己创建的每一个 spill 文件，在 dispose 时删除仍未被移除的那些，同步退出阶段则删除整个私有 spill 目录。宿主退出最终清理不创建 Promise 或定时器，保留宿主退出码与诊断，分别包含每个目标的失败，也不会声称已经完全停稳。
...
- **Windows 进程树支持仅为尽力而为**——终止经由身份围栏清扫完成（存活根用 `taskkill /PID <pid> /T /F`；已退出根的幸存后代逐一以创建身份再次核验），在所有可等待路径上经异步 taskkill spawn 投递，而退出阶段保留同步形式，等待边界仍是直接子进程：最终清扫之后才启动的后代、或在双重复用的根 pid 之下的后代，会逃出覆盖范围。
