...
A spawn builds the scrubbed child environment, starts the detached process, attaches collectors to the collected streams, and returns a handle. `done` settles at process close after a bounded pipe-drain grace, so a surviving descendant that inherited a pipe cannot hold the outcome open indefinitely; Windows termination is an identity-fenced sweep — the live root's tree, or the surviving descendants of a root that already exited — delivered through an asynchronous taskkill spawn so a cancellation tier never blocks the event loop, with the synchronous form reserved for the host-exit phase; and the escalation timer survives direct-child settlement until a scan finds nothing left to reach. Terminal cleanup sweeps descendants by exact identity, stops the shell, re-sweeps, and verifies absence through the process table.

### Safety invariants

Spill files are opened `0600` with `O_EXCL` and random names under a `0700` per-process directory, defeating symlink planting in shared temp dirs; a failed final close withholds the spill path. Process identities carry start times, so cleanup never follows PID reuse; Windows tree termination re-verifies each candidate's start identity immediately before its taskkill. The runtime registers every spill file it creates, unlinks the ones still tracked during disposal, and the synchronous process-exit phase removes the whole private spill directory. Host-exit finalization creates no promises or timers, preserves the host exit code and diagnostic, contains each target's failure, and does not claim quiescence.
...
- **Windows tree support is best-effort** — termination routes through an identity-fenced sweep (`taskkill /PID <pid> /T /F` for a live root; the surviving descendants of an exited root, each re-verified by creation identity), delivered through an asynchronous taskkill spawn on every awaitable path while the exit phase keeps the synchronous form, and the wait boundary stays the direct child: a descendant spawned after the final sweep, or under a double-reused root pid, escapes.
...
- **The credential scrub is a name heuristic** — whole underscore-delimited segments (`*_KEY`, `*_SECRET`, `*_TOKEN`, `*_PASSWORD`, `*_CREDENTIAL`, and prefixed shapes); differently named secrets (for example `*PASSPHRASE*`) pass through, and an allowlist (`SCRUB_ALLOWED_ENV_KEYS`) records exceptions. Dropped key names (never values) print on the `dsh-subprocess:env-scrub` Node debug channel.
